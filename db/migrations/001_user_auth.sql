...
ALTER TABLE users ADD COLUMN IF NOT EXISTS user_tier TEXT NOT NULL DEFAULT 'follower'
    CHECK (user_tier IN ('follower', 'voter', 'debater', 'moderator', 'sysadmin'));

DO $$ BEGIN
  ALTER TABLE users ADD CONSTRAINT users_has_auth
    CHECK (password_hash IS NOT NULL OR access_code IS NOT NULL) NOT VALID;
